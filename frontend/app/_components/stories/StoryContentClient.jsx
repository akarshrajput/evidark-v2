...
export default function StoryContentClient({ initialStory }) {
  ...
  const [contentLoaded, setContentLoaded] = useState(!!initialStory.content && !initialStory.requiresAuth);
  ...
  const shouldShowContentForAuth = !loading && isAuthenticated && (shouldShowContent || fetchingContent);
  ...
  useEffect(() => {
    if (!loading && isAuthenticated && !viewCounted && story?._id && shouldShowContent) {
      const trackView = async () => {
        try {
          const baseUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:5050";
          const token = localStorage.getItem("token");

          const response = await fetch(`${baseUrl}/api/v1/stories/${story._id}/view`, {
            method: "POST",
            headers: {
  ...
  if (loading || (isAuthenticated === undefined) || (!contentLoaded && !fetchingContent)) {
    return (
      <Card className="bg-card/50 backdrop-blur-sm border-border mb-8">
        <CardContent className="p-8">
          <div className="animate-pulse space-y-4">
      ...
      <Card className="bg-card/50 backdrop-blur-sm border-border mb-8">
        <CardContent className="p-8">
          ...
          {story.contentPreview && (
            <div className={`${merriweather.className} prose prose-invert prose-lg max-w-none leading-relaxed text-lg mb-6`}>
          ...
          <div className="relative">
            ...
            <div className="filter blur-sm pointer-events-none select-none">
              <div className={`${merriweather.className} prose prose-invert prose-lg max-w-none leading-relaxed text-lg opacity-30`}>
