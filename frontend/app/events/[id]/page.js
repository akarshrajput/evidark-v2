...
export default function EventPage() {
  ...
  const joinEventMutation = useMutation({
    mutationFn: async () => {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/events/${id}/join`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
  ...
  const leaveEventMutation = useMutation({
    mutationFn: async () => {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/events/${id}/leave`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json'
  ...
  const submitMutation = useMutation({
    mutationFn: async (content) => {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/events/${id}/submit`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json'
  ...
  const getStatusColor = (status) => {
    switch (status) {
      case 'upcoming': return 'bg-blue-900/30 text-blue-200';
      case 'active': return 'bg-green-900/30 text-green-200';
  ...
  const canSubmit = event.status === 'active' && event.isParticipant && !event.userSubmission;
            ...
            <div className="bg-card/50 backdrop-blur-sm border border-border/50 rounded-xl p-8 shadow-lg">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <div className="text-center p-3 bg-background/50 rounded-lg">
                  ...
                  <div className="text-lg font-semibold text-foreground">{event.participants?.length || 0}</div>
                  <div className="text-xs text-muted-foreground">Participants</div>
                </div>
                <div className="text-center p-3 bg-background/50 rounded-lg">
                  <Send className="w-5 h-5 text-green-400 mx-auto mb-1" />
                  <div className="text-lg font-semibold text-foreground">{event.submissions?.length || 0}</div>
                  <div className="text-xs text-muted-foreground">Submissions</div>
                </div>
                <div className="text-center p-3 bg-background/50 rounded-lg">
                  <Eye className="w-5 h-5 text-purple-400 mx-auto mb-1" />
                  <div className="text-lg font-semibold text-foreground">{event.stats?.views || 0}</div>
                  <div className="text-xs text-muted-foreground">Views</div>
                </div>
                <div className="text-center p-3 bg-background/50 rounded-lg">
                  <Trophy className="w-5 h-5 text-yellow-400 mx-auto mb-1" />
                  <div className="text-lg font-semibold text-foreground">{event.rewards?.points || 0}</div>
            ...
            <div className="bg-card/50 backdrop-blur-sm border border-border/50 rounded-xl p-6 shadow-lg">
              ...
              {event.requirements && (
                <div className="mt-6 p-4 bg-background/50 rounded-lg">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {event.requirements.description || 'No specific requirements'}
                  </p>
              ...
              {event.rewards && (
                <div className="mt-4 p-4 bg-gradient-to-r from-yellow-950/20 to-orange-950/20 border border-yellow-800/30 rounded-lg">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {event.rewards.description || `Earn ${event.rewards.points} points for participation`}
                  </p>
                </div>
              )}
            </div>
            ...
            {event.userSubmission && (
              <div className="bg-card/50 backdrop-blur-sm border border-green-600/30 rounded-xl p-6 shadow-lg">
                ...
                <div className="bg-background/50 rounded-lg p-4">
                  <p className="text-foreground whitespace-pre-wrap">{event.userSubmission.content}</p>
                  <div className="mt-3 text-sm text-muted-foreground">
                    Submitted {formatDistanceToNow(new Date(event.userSubmission.submittedAt), { addSuffix: true })}
                  </div>
                </div>
              </div>
            )}
